defmodule Av3ElixirWeb.ExpenseLive.FormComponent do
  use Av3ElixirWeb, :live_component
  ...
  defp save_expense(socket, :edit, expense_params) do
    case Expenses.update_expense(socket.assigns.expense, expense_params) do
      {:ok, _expense} ->
        {:noreply,
         socket
         |> put_flash(:info, "Depesa atualizada com sucesso")
  ...
  defp save_expense(socket, :new, expense_params) do
    case Expenses.create_expense(expense_params) do
      {:ok, _expense} ->
        {:noreply,
         socket
         |> put_flash(:info, "Depesa criada com sucesso")
         |> push_redirect(to: socket.assigns.return_to)}
